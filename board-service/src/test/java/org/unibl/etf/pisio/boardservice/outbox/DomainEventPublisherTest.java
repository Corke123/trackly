...
class DomainEventPublisherTest {
    ...
    void publish() {
        TicketCreated event = new TicketCreated(100L, 1L, 2L, "Title", "actor-1",
                Instant.parse("2026-07-25T10:00:00Z"));
        when(jsonMapper.writeValueAsString(event)).thenReturn("{\"ticketId\":100}");

        domainEventPublisher.publish(event);

        verify(outboxRepository).save(new OutboxEntry(null, "Ticket", "100", TicketCreated.TYPE,
                "{\"ticketId\":100}", event.occurredAt(), false));
    }

    @Test
    @DisplayName(
            """
            Given an event that fails to serialize, \
            when publish is called, \
            then IllegalStateException is thrown and nothing is saved\
            """)

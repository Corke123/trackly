...
class ActivityControllerIntegrationTest {
    ...
    @DisplayName(
            """
            Given recorded activity on several boards, \
            when GET /activity is called, \
            then the whole feed is returned newest first\
            """)
    void feedAcrossBoards() {
        Activity older = save(10L, TicketCreated.TYPE, "Ticket #100 created: Write tests", "actor-1",
        ...
        assertThat(response).containsExactly(
                new ActivityView(newer.id(), 20L, TicketMoved.TYPE, "Ticket #101 moved to swimlane 20", "actor-2",
                        OCCURRED_AT),
                new ActivityView(older.id(), 10L, TicketCreated.TYPE, "Ticket #100 created: Write tests", "actor-1",
                        OCCURRED_AT.minusSeconds(60))
        );
    }
    ...
    @DisplayName(
            """
            Given recorded activity on several boards, \
            when GET /activity?boardId is called, \
            then only that board's feed is returned newest first\
            """)
    void feedForSingleBoard() {
        ...
        assertThat(response).containsExactly(
                new ActivityView(newer.id(), 10L, TicketAssigned.TYPE, "Ticket #100 assigned to assignee-9",
                        "actor-1", OCCURRED_AT),
                new ActivityView(older.id(), 10L, TicketCreated.TYPE, "Ticket #100 created: Write tests",
                        "actor-1", OCCURRED_AT.minusSeconds(60))
        );
    }
